...
static inline int ai_should_log(int prio) {
    // Feeder patch: __android_log_is_loggable is only available from API 30, but the app's
    // minSdk is 29. Below 30, fall back to a compile-time level check so the native lib still
    // builds (and loads) on API 29. From 30+ the real runtime check is used.
